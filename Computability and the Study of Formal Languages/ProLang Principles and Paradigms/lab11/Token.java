...
public class Token
{
  ...
  private Token(TokenType tok, String s) {
    this.type = tok;
    this.value = s;
    if (tok.compareTo(TokenType.Eof) < 0) {
      int i = tok.ordinal();
      reserved[i] = s;
      token[i] = this;
    }
  ...
  public static Token keyword(String s) {
    int i = s.charAt(0);
    if ((i >= 65) && (i <= 90)) return mkIdentTok(s);
    for (int j = 0; j < KEYWORDS; j++)
      if (s.equals(reserved[j])) return token[j];
    return mkIdentTok(s);
  }

  public static Token mkIdentTok(String s) {
    return new Token(TokenType.Identifier, s);
  }

  public static Token mkIntLiteral(String s) {
    return new Token(TokenType.IntLiteral, s);
  }

  public static Token mkFloatLiteral(String s) {
    return new Token(TokenType.FloatLiteral, s);
  }

  public static Token mkCharLiteral(String s) {
    return new Token(TokenType.CharLiteral, s);
  }

  public String toString() {
    if (this.type.compareTo(TokenType.Identifier) < 0) return this.value;
